...
internal class ChartContentView: UIView {
    ...
    init(chartView: ChartView) {
        ...
        if #available(iOS 13.0, *) {
            visualEffectView.effect = UIVibrancyEffect(blurEffect: UIBlurEffect(style: .systemMaterial))
        }
...
internal class ChartBackgroundView: UIView {
    ...
    func drawAxisLabelsAndGridlinesInContext(_ context: CGContext, chartView: ChartView) {
        ...
        // Used to round strided values to the same maximum fractional digits as the number formatter
        ...
        if let chartSafeAreaLayoutGuide = chartView.chartSafeAreaLayoutGuide {
            xAxisLabelMinY = chartSafeAreaLayoutGuide.layoutFrame.origin.y
        }
        else {
        ...
        for ((x, xScreenPos), xLabelImage) in zip(xValues, xLabelImages) where abs(x) > Double.ulpOfOne {
        ...
        for ((y, yScreenPos), yLabelImage) in zip(yValues, yLabelImages) where abs(y) > Double.ulpOfOne {
