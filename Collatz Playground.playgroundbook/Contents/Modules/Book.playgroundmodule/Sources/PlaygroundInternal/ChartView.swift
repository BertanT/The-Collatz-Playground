...
internal class ChartView: UIView, UIScrollViewDelegate, UIGestureRecognizerDelegate {
    ...
    var defaultScrollViewContentOffset: CGPoint {
        ...
        let frameSize: CGSize = scrollView.frame.size
        
        return CGPoint(x: (contentSize.width / 2.0) - (frameSize.width / 2.0), y: (contentSize.height / 2.0) - (frameSize.height / 2.0))
    }
    ...
    internal func removeChartDrawable(chartDrawable: ChartDrawable) {
        let index = chartDrawables.firstIndex { otherChartDrawable -> Bool in
