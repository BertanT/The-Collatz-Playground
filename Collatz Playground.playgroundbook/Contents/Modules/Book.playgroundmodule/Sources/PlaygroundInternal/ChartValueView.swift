...
internal class ChartValueView: UIView {
    ...
    private func targetLayoutFor(calloutView: CalloutView, relativeTo otherCalloutViews: [CalloutView], anchorPoint: CGPoint, preferredDirections: [CalloutDirection]) -> CalloutLayout? {
        ...
        if let previousTargetLayout = calloutView.targetLayout {
            if let index = directionsToAttempt.firstIndex(of: previousTargetLayout.direction) {
...
private class CalloutView: UIView {
    ...
    private class CalloutContentView: UIView {
        ...
        func updateContent() {
            ...
            let decimalPointX = String.localizedStringWithFormat("%.\(decimalPoints)f", chartValue.value.0)
            let decimalPointY = String.localizedStringWithFormat("%.\(decimalPoints)f", chartValue.value.1)
            ...
            if displayIndex {
                ...
                xValue = formatter.string(from: NSNumber(value: chartValue.value.0)) ?? decimalPointX
                yValue = formatter.string(from: NSNumber(value: chartValue.value.1)) ?? decimalPointY
            } else {
                xValue = decimalPointX
                yValue = decimalPointY
            }
            ...
            xTitleLabel.attributedText = NSAttributedString(string: NSLocalizedString("x:", comment: "label for the x value"), attributes: fieldTitleAttributes)
            xValueLabel.attributedText = NSAttributedString(string: xValue, attributes: valueLabelAttributes)
            
            yTitleLabel.attributedText = NSAttributedString(string: NSLocalizedString("y:", comment: "label for the y value"), attributes: fieldTitleAttributes)
